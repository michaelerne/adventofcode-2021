...
def parse_data(data):
    return [
        ...
        for instr, x_from, x_to, y_from, y_to, z_from, z_to in parse.findall('{:w} x={:d}..{:d},y={:d}..{:d},z={:d}..{:d}', data)
    ]
...
def get_overlap(r1, r2):
    return [
        get_subrange(sr1, sr2)
        for sr1, sr2 in zip(r1, r2)
    ]


def count_unique(instruction, remaining_instructions):
    _, instruction_ranges = instruction

    total = math.prod([len(r) for r in instruction_ranges])

    overlaps = []
    for state, remaining_instruction_ranges in remaining_instructions:

        overlap_ranges = get_overlap(instruction_ranges, remaining_instruction_ranges)

        if all([len(r) != 0 for r in overlap_ranges]):
            overlaps.append((state, overlap_ranges))

    while overlaps:
        overlap, *overlaps = overlaps
        total -= count_unique(overlap, overlaps)

    return total


def part_a(data):
    remaining_instructions = parse_data(data)
...
on x=10..10,y=10..10,z=10..10""", 39, None),
